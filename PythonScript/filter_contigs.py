...
def parse_xmap_entry(line):
    ...
    if abs(query_start_pos - query_end_pos) > 45000 and (
            (12 > abs(query_start_index - query_end_index) > 9 and score > 5000) or (
            16 > abs(query_start_index - query_end_index) > 11 and score > 4500) or (
                    abs(query_start_index - query_end_index) > 15 and score > 4500)):
        contig_alignments[contig_id].append((score, line))
...
def filter_contig_alignments():
    for k in contig_alignments.keys():
        ...
        for i in range(len(contig_alignments[k])):
            ...
            for j in range(i + 1, len(contig_alignments[k])):
                alignment_j = contig_alignments[k][j]
                alignment_j_line = alignment_j[1].split('\t')
                # chech if alignment i segment on contigs contain alignment j segment of contigs, ignore j
                if min(float(alignment_i_line[3]), float(alignment_i_line[4])) - 2000 <= min(float(alignment_j_line[3]),
                                                                                             float(alignment_j_line[
                                                                                                       4])) and max(
                    float(alignment_i_line[3]),
                    float(alignment_i_line[
                              4])) + 2000 >= max(
                    float(alignment_j_line[3]), float(alignment_j_line[4])):
                    deleted.append(alignment_j)

                # if j contain i (with lower score) but difference between length i and j is greater than 50000, ignore i(high_score)
                if min(float(alignment_j_line[3]), float(alignment_j_line[4])) <= min(float(alignment_i_line[3]),
                                                                                      float(alignment_i_line[
                                                                                                4])) and max(
                    float(alignment_j_line[3]),
                    float(alignment_j_line[4])) >= max(
