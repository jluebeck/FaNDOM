                ...
                for v in range(0, len(alignment) - 1):
                    ...
                    if kk * scale < ref[ref_id][ref_1] <= (kk + 1) * scale:
                        ...
                        if abs(q_2-q_1) < 20 and abs(ref_2-ref_1)< 20:
                            ...
                            dif = ref_dist - q_dist
                            if dif < -2000:
                                if mol[q_id][min(q_1, q_2)] > min(0.25 * mol[q_id][len(mol[q_id])],10000) and mol[q_id][max(q_1, q_2)] < max(0.75 * mol[q_id][len(mol[q_id])],  mol[q_id][len(mol[q_id])]-10000):
                                    d_score = 0
                                    ans['insertion'].append([q_id, ref_1, ref_2, q_1, q_2, dif, d_score])
                            if dif > 2000:
                                if mol[q_id][min(q_1, q_2)] > min(0.25 * mol[q_id][len(mol[q_id])],10000) and mol[q_id][max(q_1, q_2)] < max(0.75 * mol[q_id][len(mol[q_id])], mol[q_id][len(mol[q_id])]-10000):
                                    d_score = 0
                                    ans['deletion'].append([q_id, ref_1, ref_2, q_1, q_2, dif, d_score])
