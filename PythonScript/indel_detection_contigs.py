                        ...
                        if kk * scale < ref[ref_id][ref_1] <= (kk + 1) * scale:
                            ...
                            if dif < -2000:  # insertion with length greater than 2Kbp
                                ...
                                if mol[q_id][min(q_1, q_2)] > min(0.25 * mol[q_id][len(mol[q_id])], 10000) and \
                                        mol[q_id][max(q_1, q_2)] < max(0.25 * mol[q_id][len(mol[q_id])],
            ...
            for i in ans:
                if len(ans[i]) > 0:  # chech is there any indel here in this region?
                        ...
                        if aln[1] >= i_j[0] - 0 and aln[2] <= i_j[1] + 0:
                            ...
                            if len(selected) > 0:
                                ...
                                lines_to_write.append(
                                    "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(i, k, ref[str(rev_contig[k])][i_j[0]],
                                                                                 ref[str(rev_contig[k])][i_j[1]], float(
                                            float(s) / len_selected_before_set), ','.join(selected),
                                                                                 len(set(selected))))
                    ...
                    if len(selected) > 0:
                        ...
                        lines_to_write.append(
                            "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(i, k, ref[str(rev_contig[k])][i_j[0]],
                                                                         ref[str(rev_contig[k])][i_j[1]],
                                                                         float(float(s) / len_selected_before_set),
                                                                         ','.join(selected), len(set(selected))))
            ...
            for j in range(len(d[k])):
                if j != i:
                    ...
                    if p[7] == direction and p[2] == chrom:
                        ...
                        if min(float(p[3]), float(p[4])) <= next_s_q <= max(float(p[3]), float(p[4])) or min(
                                float(p[4]), float(p[3])) <= next_e_q <= max(float(p[3]), float(p[4])):
                            align_p = p[-1].split('(')[1:]
                            ...
                            for pair in selected_pair:
                                if str(next_s) == pair.split(',')[1][:-1]:
                                    ...
                                    if abs(float(s_ref) - float(s_ref_p)) - abs(
                                            next_s_q - mol[str(k)][s]) > 2000 and abs(
                                            float(s_ref) - float(s_ref_p)) < 300000:
                                        lines_to_write.append(
                                            "deletion\t{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".format(contig_id_map[int(chrom)],
                                                                                              s_ref_p, s_ref, abs(
                                                    float(s_ref) - float(s_ref_p)) - abs(next_s_q - mol[str(k)][s]), k,
                                                                                              1))
                                    x1 = 1
                                if str(next_e) == pair.split(',')[1][:-1]:
                                    e_ref_p = ref[str(chrom)][int(pair.split(',')[0])]
                                    if abs(float(e_ref) - float(e_ref_p)) - abs(
                                            next_e_q - mol[str(k)][e]) > 2000 and abs(
                                            float(e_ref) - float(e_ref_p)) < 300000:
                                        lines_to_write.append(
                                            "deletion\t{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".format(contig_id_map[int(chrom)],
                                                                                              e_ref, e_ref_p, abs(
                                                    float(e_ref) - float(e_ref_p)) - abs(next_e_q - mol[str(k)][e]), k,
                                                                                              1))
                                    x2 = 1
                                if x1 + x2 == 2:
                                    break
